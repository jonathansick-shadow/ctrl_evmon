...
public class TestCase25 {
	...
	public Job createJob() {
		...
		String query = "SELECT date, nanos, id, sliceid, runid, level, log, custom, hostid, status, pipeline from logger where runid='"
				+ runId + "' and log='harness.slice.visit.stage.handleEvents.eventwait' order by nanos;";

		String logName = "harness.slice.visit.stage.handleEvents.eventwait";
		...
		// write to database
		String insertQuery = "INSERT INTO test_events.durations_eventwait(runid, name, sliceid, duration, host, loopnum, pipeline, date, stageid) values({$msg:runid}, {$msg:log}, {$msg:sliceid}, {$duration}, {$msg:hostid}, {$firstLoop}, {$msg:pipeline}, {$startdate}, {$msg:stageId});";
